...
public class SourceExport {
	public static void exportSource(IJavaProject javaProject,
		...
		String DB_URL = "jdbc:hsqldb:file:" + targetLocation + "/jenaDB"; // URL of database
		...
		conn.cleanDB(); // it doesn't seem to work without this -- TODO: figure out why
		ModelMaker maker = ModelFactory.createModelRDBMaker(conn);

		// create or open the default model
		Model model = maker.createDefaultModel();
		extractGraph(javaProject, model);
	...
	private static void extractGraph(IParent parent, final Model model) throws JavaModelException {
		if(parent instanceof IPackageFragment) {
			IPackageFragment packageFragment = (IPackageFragment) parent;
			model.createResource(packageFragment.getElementName()).addProperty(Properties.TYPE, Types.PACKAGE);
		}
		for (int i = 0; i < parent.getChildren().length; i++) {
			final IJavaElement element = parent.getChildren()[i];
			if(parent instanceof IPackageFragment) {
				IPackageFragment packageFragment = (IPackageFragment) parent;
				model.createResource(packageFragment.getElementName()).addProperty(Properties.CONTAINS, element.getElementName());
			}
			if (element instanceof ICompilationUnit) {
				...
				final Resource elementRes = model.createResource(element.getElementName());
				...
				result.accept(new ASTVisitor() {
					...
					public boolean visit(TypeDeclaration node) {
						...
						Resource currentRes = model.createResource(typeBinding.getQualifiedName()); // TODO: figure out how to get the package
						if(node.isInterface()) {
					...
					public boolean visit(MethodDeclaration node) {
						...
						Resource currentRes = model.createResource(typeBinding.getQualifiedName() + "." + methodBinding.getName() + "(..)");
					...
					public boolean visit(MethodInvocation node) {
						...
						ITypeBinding typeBinding = methodBinding.getDeclaringClass();
						getTop().addProperty(Properties.CALLS, typeBinding.getQualifiedName() + "." + methodBinding.getName() + "(..)");
						return true;
					}
				});
			}
			if (element instanceof IParent) {
				if (!element.getElementName().endsWith("jar")) {
					extractGraph((IParent) element, model);
				}
			}
		}
	}
