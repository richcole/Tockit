...
/*        int initialPosition = (int)Math.floor(partitionY/2) * partitionX +
                              (int)Math.floor(partitionX/2);*/
        ...
        if (typeOfObject==1) {
            ...
            for (int p=4;midGridPosition - partitionX * p>=0;p+=4) {
                if (getGridPointStatus(midGridPosition - partitionX * p) == 1) {
                    return (Point2D.Double) gridPoints.get(midGridPosition - partitionX * p);
                }
            }

            for (int p=4;midGridPosition + partitionX * p <= gridSize;p+=4) {
                if (getGridPointStatus(midGridPosition + partitionX * p) == 1) {
                    return (Point2D.Double) gridPoints.get(midGridPosition + partitionX * p);
                }
            }
        ...
        if (typeOfObject == 2) {
/*
            if (getGridPointStatus(midGridPosition - 12) == 1) {
                return (Point2D.Double) gridPoints.get(midGridPosition - 12);
            }

            for (int p=4;(gridView.getMidGridPosition()-12) - gridView.getPartitionX() * p>=0;p+=4) {
                if (gridView.getGridPointStatus((gridView.getMidGridPosition()-12) -
                                                gridView.getPartitionX() * p) == 1) {
                    nodeView.setPosition(gridView.getNextAvailableGridPoint((gridView.getMidGridPosition()-12) -
                                                                            gridView.getPartitionX() * p));
                }

                if (!node.hasPosition()) {
                    for (p=4;(gridView.getMidGridPosition()+12) + gridView.getPartitionX() * p <=
                             gridView.getPartitionX() * gridView.getPartitionY();p+=4) {
                        if (gridView.getGridPointStatus((gridView.getMidGridPosition()+12) +
                                                        gridView.getPartitionX() * p) == 1) {
                            nodeView.setPosition(gridView.getNextAvailableGridPoint(
                                                 (gridView.getMidGridPosition()+12) +
                                                 gridView.getPartitionX() * p));

        }
*/
        int XGridViewPosition = (int)Math.IEEEremainder(midGridPosition, partitionX) + 1;
//        int YGridViewPosition = (int)Math.IEEEremainder(objPosition, partitionY) + 1;
        ...
        return null;
/*
        for (int i=0; i<=gridPointsStatus.length;i++) {
            if (gridPointsStatus[i] == 1) {
                updateGridPointStatus(i,0);
                return (Point2D.Double) gridPoints.get(i);
            }
        }
        return null;
*/
