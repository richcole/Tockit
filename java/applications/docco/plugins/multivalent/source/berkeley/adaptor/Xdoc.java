...
class FontXdoc extends Font {
  ...
  FontXdoc(String name, int style, int size, int capheight, int descend) {
	super(name, style, size);
	this.capheight = (capheight>0? capheight : (int)(((double)size)*1.25));
...
//System.out.println("capheight="+this.capheight+", descend="+this.descend);
  ...
  public String toString() {
	return ""+name+" "+capheight+"/"+descend+", "+sstyle[style]+", "+size;
  }
}
...
*/
public class Xdoc extends multivalent.std.adaptor.MediaAdaptorByte {
  /*final--available to command line*/ boolean DEBUG = false;
  ...
  /**
	Set showing verbose XDOC information: on/off as parsed by {@link phelps.lang.Booleans#getBoolean(String, boolean)}, or <code>null</code> to toggle.
	<p><tt>"verboseXDOC"</tt>.
  */
  ...
  static final double MM2PPI = 300.0/254.0;

  // word character translation
  static String transString[] = new String[256];
  ...
  static final String specialChar[] = { "`", "'", "``", "''", "--", "*"/*bullet*/, "\207", "\210", "\211", "\212" };
  static {
  ...
  double scale = 3.335; //1.0; 3.31
  //Image fullpage_ = null;
  ...
  static final DecimalFormat DLFORMAT = new DecimalFormat("00000000");
  int charcnt_ = 0;
  public int getCharCnt() { return charcnt_; }
  int qcharcnt_ = 0; // count questionable characters to find bad scans, math
...
//System.out.println("extracting Xdoc page #"+page);
	...
	try {
		...
		BufferedReader r = new BufferedReader(new InputStreamReader(biburi.toURL().openStream()));
		String line;
...
//System.out.println("scale from bib = "+scale);
  ...
  public void setInputStream(InputStream is) {
	is_ = new PushbackInputStream(is, 1);
	super.setInputStream(is_);
  }


  public void buildBefore(Document doc) {
	if (doc.getAttr(Document.ATTR_PAGE)==null) { new LeafAscii(""/*"Loading XDOC..."/*no PAGE attr for XDOC*/, null, doc); return; }
...
//	if ((att=/*new*/doc.getAttr("mode"))!=null && att.equals("ASCII")) active=1;
	...
	try {
		...
		this.setInputStream(xdcuri.toURL().openStream());
		/*ocrroot_ = (FixedI)*/parse(/*xdcuri.toURL().openStream(),*/ doc);
	} catch (FileNotFoundException fnfe) {
		new LeafAscii("no such page",null, doc);
		return;
	} catch (Exception e) {
...
//	super.buildBefore(doc); // have to set ocrroot first
  ...
  public boolean paintAfter(Context cx, Node node) {
	...
	if (getRoot().getAttr(ATTR_VERBOSE)!=null) {
		...
		int x = getDocument().bbox.width - 100, h = g.getFontMetrics().getAscent()/*+2*/;
...
//System.out.println("full image = "+fullpage_.getClass().getName()+" instanceof Buffer "+ (fullpage_ instanceof java.awt.image.BufferedImage)); => FALSE
  ...
  void adjustLeaf(FixedLeafOCR n) {
  ...
  /** For rules and text */
  void addWord(FixedI tome, String name, Rectangle bbox, FontXdoc font) {
...
*/
	...
	l.getIbbox().y -= font.capheight; // computed on the fly from baseline
	l.getIbbox().height = font.capheight + font.descend + 2;
	l.font = font;
  ...
  void addWord(FixedI tome, String name, Rectangle bbox) {
...
//System.out.println("******** XDOC PARSE");// #"+(++parsecnt));
	...
	fontlist[0] = new FontXdoc("Serif", Font.PLAIN, 12, 0, 0); // sometimes this is missing
...
//System.out.println("xdoc version = "+xdocvers);
			...
			case 'f': // font information
				...
				// convert to Java Font information -- I think this is bogus
/*				String jfontname = null;
				int inx = (fontname!=null? Arrays.binarySearch(StyleSheet.LCFAMILIES, fontname.toLowerCase()): -1);
//System.out.println("font = |"+fontname+"|");
				if (inx>=0) { jfontname = StyleSheet.FAMILIES[inx]; /*System.out.println("fam=>"+StyleSheet.FAMILIES[inx]);* / }*/
				String jfontname = phelps.awt.font.Fonts.getNormalFamily(fontname);
				if (jfontname==null) jfontname = (fv=='F'? "Monospaced": "Serif");

				int jstyle = Font.PLAIN;
				switch (style) {
				case 'R': jstyle = Font.PLAIN; break;
				case 'B': jstyle = Font.BOLD; break;
				case 'I': jstyle = Font.ITALIC; break;
				case 'T': jstyle = Font.BOLD|Font.ITALIC; break;
				default:
					//throw new ParseException("unrecognized font style "+style, style);
					//if (DEBUG) System.err.println("unrecognized font style "+style);
					assert false: (char)style;
					//jstyle = Font.PLAIN;
				}
				//curfont/*don't always get a 'c' first*/ => taken from 's'
//if (fontid>=fontlist.length) System.out.println("BIG FONTID! "+fontid); else
				fontlist[fontid] = new FontXdoc(jfontname, jstyle, points, capheight, lowerw-lowerwo);
...
//		if (regionorder[i]!=-1) root.appendChild(regiony[i]);
	...
	if (xdocvers<10.0) {
		for (FixedLeafOCR n=(FixedLeafOCR)root.getFirstLeaf(), endn=(FixedLeafOCR)root.getLastLeaf().getNextLeaf(); n!=endn && n!=null; n=(FixedLeafOCR)n.getNextLeaf()) {
			...
			FontXdoc xfont = (FontXdoc)fontlist[ibbox.height];
  ...
  public static void main(String argv[]) {
	...
	for ( ; argi<argv.length && argv[argi].charAt(0)=='-'; argi++) {
	  ...
	  if (arg.equals("dump")) {/**/}
		...
		if (percentbad>=1.0) percentbad /= 100.0;
	  }
	}

	// use System.in -- for now hack default
	if (argi==argv.length) {
	  System.err.println("usage: java Xdoc [-dump] [-debug] [-bad <%age>] <file1.xdc> [<file>...]");
	  System.exit(1);
	}

	// don't bring system to its knees if testing lotsa files
	Thread.currentThread().setPriority(Thread.MIN_PRIORITY);

	// list of XDOC files
	for (; argi<argv.length; argi++) { // => use Files.depthFirstIterator
	  Xdoc xdoc = new Xdoc();	// probably not necessary, but just in case
	  xdoc.Test = true;
	  xdoc.DEBUG = debug;
...
/*
	  if (debug) System.out.println("parse Xdoc for "+uri);
	  try {
		xdoc.parse(null, null);
	  } catch (Exception e) {
		System.err.println("\t"+e);
		continue;
	  }*/

	  double actualbad = xdoc.getQuestionableCharCnt()/(double)xdoc.getCharCnt();
	  if (actualbad > percentbad) System.out.println("many questionable characters "+actualbad*100.0);

//	  if (dump) xdoc.ocrroot_.dump();
	}
  }
}
