...
public class OcrView extends Behavior implements ImageObserver {
  ...
  */
  ...
  static Font smallfont = new Font("Serif", Font.ITALIC, 12);
  static Font bigfont = new Font("Serif", Font.BOLD, 24);


  // background can be full page or array of images
  //Image fullpage = null;
  FixedLeafImage[] frags = null;
...
if (rootStyle_==null) { System.out.println("rootStyle_=null in semEvAf");  }
  ...
  public void buildAfter(Document doc) {
	...
	if (o instanceof FixedLeafImage) { /*fullpage = (Image)o;*/ frags = new FixedLeafImage[1]; frags[0]=(FixedLeafImage)o; }
	else if (o instanceof List) frags = (FixedLeafImage[])((List)o).toArray(new FixedLeafImage[((List)o).size()]);
	//else null or error
...
//System.out.println("adding image dim to "+w+"x"+h);
	...
	if (frags!=null) for (int i=0,imax=frags.length; i<imax; i++) {
		FixedLeafImage l = frags[i];
...
//System.out.println("OcrView mode = "+ocrSignal);
	if (ocrroot_.size()==0) {
		g.setFont(bigfont);
		g.drawString("This page intentionally left blank", 20, 100);
		return true;
...
*/
		...
		else*/ if (frags!=null) for (int i=0,imax=frags.length; i<imax; i++) {
			//frags[i].paintNode(null, cx); => node just packaging
			FixedLeafImage l = frags[i];
...
//System.out.println("fullpage in OcrView "+fullpage.getWidth(this)+"x"+fullpage.getHeight(this));
		if (doc.getAttr(Fixed.ATTR_REFORMATTED)==null) {
			...
			g.setFont(smallfont);
			g.drawString("Parts of image not recognized by OCR not shown; use View/Full Image Only to see.", 10, 15);   // discard unless PDF's recognition engine is as retarded as XDOC's
		}
