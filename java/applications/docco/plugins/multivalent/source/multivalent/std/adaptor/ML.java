...
public abstract class ML extends MediaAdaptorReader {
  ...
  private static Map entityML_ = new HashMap(10);
  static {
	String[] en = {
		...
		if (ch==escchar && ch!='%') {	// FIX!
			...
			if (escchar=='&') {
				...
				if (ref.length()==0) trans="&";    // user mistakenly wrote '&' instead of '&amp;' -- I guess this is an error case in straight SGML
					...
					if (ref.charAt(1)=='x' || ref.charAt(1)=='X') try { trans=String.valueOf((char)Integer.parseInt(ref.substring(2,0x10))); } catch (NumberFormatException e) { trans=ref; }
