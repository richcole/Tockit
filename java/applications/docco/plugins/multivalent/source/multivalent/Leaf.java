...
/*!!!abstract*/ public class Leaf extends Node {
  ...
  */
  public Leaf(String name, Map attr, INode parent) { super(name, attr, parent); }
  ...
  */
  /*final--but HTMLHR and ShadowLeaf*/ public boolean formatNode(int width, int height, Context cx) {
...
//System.out.println((m==r.getStart()?"add":"remove")+"Span "+r+" at "+this+"."+start+" => "+cx.vactive_);
	...
	if (nW==0) { bbox.height = baseline = 0; }
...
//	abstract int size();
  ...
  public void paintBeforeAfter(Rectangle docclip, Context cx) {
	// "&&" because sometimes have 0-width for cursor in 0-length content
	...
	if (!intersects(docclip) || (bbox.width==0 && bbox.height==0)) {
		// gc invalid only if spans that start or stop within subtree
...
//System.out.println("me="+cbbox+" "+docclip);
  ...
  /*final--ShadowLeaf/* until reason to do otherwise*/ public void paintNode(Rectangle docclip, Context cx) {
	...
	cx.x = 0; cx.baseline = baseline + bbox.y;
	int start=0, end;
	boolean completecontext;

	Behavior[] paintbefore = (stickycnt_==0? null: new Behavior[stickycnt_]);
	int paintbeforecnt = 0;
	boolean elidebefore = cx.elide; int xbefore=0;//bbox.x;    // special cases
	StyleSheet ss = cx.styleSheet;

	Span lastspan = null;
	List actives = cx.vactive_;  // can't change during because don't recurse to anybody that recomputes
	for (int i=0,imax=stickycnt_; i<imax; ) {
		// maybe give ranges their own ArrayList subclass if casting is a performance problem
...
//System.out.println("paintbeforecnt = "+paintbeforecnt);
	if (!elidebefore && paintbeforecnt>0) {
		boolean tmpelide = cx.elide; cx.elide = elidebefore;
		int tmpx=cx.x; cx.x = (xbefore>=bbox.width && xbefore>0? xbefore-1: xbefore);
  ...
  protected synchronized void subelement() {
	if (this!=cacheLeaf_) {
		...
		//Context cx = (ss!=null? ss.getContext(null): new Context(getBrowser()));
...
if (cx==null /*&& valid_*/) dump();
  ...
  */
  ...
  /*final--scrollbar*/ public boolean eventNode(AWTEvent e, Point rel) {
...
/*
	...
	List vrangeActive = br.getActives();
...
//System.out.println("leaf = "+n.getName());
			length += INTERWORD_SPACE + n.size();	 // space before this word + this word
		}
		length += INTERWORD_SPACE+endi;
	}
...
*/
  ...
  // structural split -- good enough for adding lines
  // use to interpose other media
  public void split(int off) {
	assert off>=0 && off<=size(): "split @ "+off+" <=? "+size();

	INode p=getParentNode(), pp=p.getParentNode();
	INode newp=null;
	try { newp=(INode)p.clone(); } catch (CloneNotSupportedException cant) { System.out.println("can't clone anymore guys! "+cant); }
...
//System.out.println("eatme "+getName()+"/"+startoff+" .. "+endn.getName()+"/"+endoff);
		...
		for (int i=n.sizeSticky()-1; i>=0; i--) {
			...
			if (m.getOwner() instanceof Span) {
				...
				if (m == r.getStart()) startm.put(r,m); // haven't seen end as start always comes before end
					...
					if (startm.get(r)!=null) { startm.remove(r); r.destroy(); }	  // seen the pair, zap it
	...
	if (startn==endn) {
		...
		for (Iterator e=startm.keySet().iterator(); e.hasNext(); ) {
			Span r = (Span)e.next();
	...
	if (scope!=null && scope.getFirstLeaf()==null) {
		for (INode pbot=scope, pnext=scope; pnext!=null; pbot=pnext) {
			...
			if (pnext==null) {
				...
				startn = this;    // don't assume it's LeafAscii, and don't depend on LeafAscii
	...
	if (cursset) {
		...
		//curs.scope = scope;
//System.out.println("cursor => "+curs.getMark().leaf.getName()+"/"+curs.getMark().offset+" w/scope "+scope);
	} //else System.out.println("cursor wasn't set?");
