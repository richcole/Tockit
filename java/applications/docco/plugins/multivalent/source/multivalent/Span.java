...
public class Span extends Behavior implements ContextListener, EventListener {
  ...
  */
  ...
  private static Map pending__ = new IdentityHashMap(100);    // pending open() spans, which requires client to close() all open(), but that's like File.close() so ok
  ...
  public Map pstart=null;  // keep persistent hooks around to update positions when save -- needed by RestoreReport
  public Map pend=null;
  ...
  /**
	...
	<p>All spans <code>open</code>ed must be <code>close</code>ed or there will be memory leaks.
  ...
  */
  public void open(Node/*not necessarily a Leaf*/ start) {
	...
	assert pending__.get(this) == null;

	Mark s = getStart();
	//while (s.isStruct()) { Node last=((INode)s).getLastChild(); if (last!=null) s=last; else break; } => may delete some (empty) nodes as build tree
	//s.leaf = start;   // can't do this because start isn't necessarily a Leaf!  maybe just put in hash iff not leaf
	/*if (start!=null)*/ pending__.put(this, start);
...
//System.out.print("open |"+start+"|/"+s.offset);
  ...
  */
  public boolean close(Node end) {
	...
	assert pending__.containsKey(this): end;
	Node s=(Node)pending__.remove(this)/*not getStart().leaf*/, e=end;
//System.out.print("attach "+getName()+": "+s+"/"+(s!=null? s.size(): -1)+" .. "+e+"/"+(e!=null? e.size(): -1));
	//if (e==null) return;
	...
	if (s!=null && e!=null) {
		...
		Leaf sl;
		if (s.isStruct()) {
//System.out.println("struct "+s+" "+si+" vs "+s.size());
...
//if (sl==null || el==null) System.out.println("0-length: "+getName());
  ...
  */
  public static int closeAll(Node subtree) {
	...
	if (end != null) synchronized (pending__) {
		List l = new ArrayList(10);

		// first collect...
		for (Iterator i = pending__.entrySet().iterator(); i.hasNext(); ) {
			Map.Entry e = (Map.Entry)i.next();
			if (subtree.contains((Node)e.getValue())) {
//System.out.println("close "+e.getKey());
		...
		for (Iterator i = l.iterator(); i.hasNext(); ) {
			boolean ok = ((Span)i.next()).close(end);
			if (ok) cnt++;
		}
	}
  ...
  */
  public void move(Leaf ln,int lo, Leaf rn,int ro) {
	...
	Browser br=getBrowser(); if (br!=null) br.repaint(250);	// can't just reformat+repaint old and new positions because reformatting may change locations
  }
  ...
  public void move(Span s) {
	...
	Browser br=getBrowser(); if (br!=null) br.repaint(250);
  }
...
//System.out.println("destroy "+getName());
	if (isSet()) {
		...
		if (isp.isValid()) getBrowser().repaint(250);   // if removing many, batch these (don't rely on Java)
		//Document doc = getDocument(); if (doc!=null && doc.) br.repaint(250);
		markDirty();
		moveq(null);    // clean up stickies
//System.out.println("move null");
	} else pending__.remove(this);     // hopefully not needed but be safe
...
//System.out.println("*** saving span "+getClass().getName());
	...
	if (isSet()) {  // => if deleted removed from layer and not called.  if not set, don't report ro lo
		...
		if (pstart==null) pstart=new CHashMap(5); else pstart.clear();
		RobustLocation.descriptorFor(startn,starti, doc, pstart);
		if (pend==null) pend=new CHashMap(5); else pend.clear();
		RobustLocation.descriptorFor(endn,endi, doc, pend);

		// record length for use when one endpoint restorable but not the other
		putAttr("length", Integer.toString(startn.lengthTo(starti, endn,endi)));
		// record contents of span, so can search web if can't find page URL
		StringBuffer csb = new StringBuffer(1000);
		if (startn instanceof LeafText) csb.append(((LeafText)startn).getText().substring(starti));
		if (endn!=startn) {
			for (Leaf n=startn.getNextLeaf(); n!=endn; n=n.getNextLeaf()) {
				if (n instanceof LeafText) csb.append(' ').append(((LeafText)n).getText());
			}
			if (endn instanceof LeafText) csb.append(((LeafText)endn).getText().substring(0,endi));
		}
		if (csb.length()>0) putAttr("content", csb.substring(0));
	}
	// NB attribute always current

	e.appendChild(new ESISNode(GI_START, pstart));
	e.appendChild(new ESISNode(GI_END, pend));
...
//System.out.println("restore "+getName()+", ch cnt="+(n!=null?n.size(): 0));
	if (n!=null) for (int i=0,imax=n.size(); i<imax; i++) {
		if (n.childAt(i) instanceof ESISNode) {
			...
			if (GI_START.equals(gi)) pstart = attrs;
			else if (GI_END.equals(gi)) pend = attrs;
		}
	}
...
//createUI("button", getClass().getName()+" in "+getLayer().getName(), "", menu, "EDIT", true);
		createUI("button", "Morph Span to Selection", new SemanticEvent(br, MSG_MORPH, null, this, null), menu, "EDIT", !br.getSelectionSpan().isSet());
		createUI("button", "Delete Span"/*+" "+getClass().getName()*/, new SemanticEvent(br, MSG_DELETE, null, this, null), menu, null/*end of menu, after protective separator*/, false);
	}
	return false;
  }
