...
public class TableA
{
    ...
    void add(String s,
             ...
             int j)
    {
        if ((i < maxXItems) && (j < maxYItems))
        {
            items[j][i] = s;
        }
        else
        {
            System.out.println("ERROR - ATTEMPT TO ACCESS ARRAY VALUE LARGER THAN IS STORED");
        }
    }
        ...
        try
        {
            ...
            while ((curBint = fullFileText.read()) != -1)
            {
                ...
                if (c == '>')
                {
                    ...
                    else if (compTag.startsWith("</td")
                                 || compTag.startsWith("</th"))
                    {
                        inCell = false;
                        s5 = s5 + c;
                ...
                int j)
    {
        try
        {
            ...
            FileWriter filewriter = new FileWriter(file);
            PrintWriter printwriter = new PrintWriter(filewriter);
            printwriter.println("<html><header><title>" + captionStr
                                + "</title></header>");
            printwriter.println("<body bgcolor=" + colorStr + "><p>"
                                + captionStr + "</p>");
            printwriter.println("<table bgcolor=#f0f0f0 border=1 bgcolor="
                                + bgColorStr + " summary=\"" + tableSumStr
                                + "\">\n");
    ...
    void fpImport(String s)
    {
        try
        {
            ...
            {
                if ((orderreader1.inItem(j2) != null)
                        && (orderreader1.inItem(j2).length() > 3))
                {
            ...
            for (; i3 < l; i3++)
            {
                ...
                for (int k3 = 0; k3 < i2; k3++)
                {
                    ...
                    if ((i3 > 0)
                            && orderreader.inItem(i3 - 1).equals(orderreader2
                                                                     .inItem(k3)))
                    {
                        flag8 = true;
                    }
                  ...
                  int j)
    {
        ...
        {
            System.out.println("ERROR - ATTEMPT TO REFERENCE ITEM THAT DOESN'T EXIST!");

            return "";
        }
    }

    Double itemVal(int i,
                   int j)
    {
        int k = 0;
        boolean flag = true;
    ...
    int colSize()
    {
        ...
        {
            System.out.println("Error - null value in table : "
                               + exception.toString());
        }
    ...
    public int inTableRow(String s,
                          int i,
                          int j)
    {
        int k = -1;
